...
public class CondimentsMenu extends MenuItems {
...
//        super(stmt);
    ...
    public void showTopItems(String nutrient) throws SQLException {
        String sql = "SELECT * FROM condiments_menu ORDER BY " + nutrient + " DESC LIMIT 5;";
        ResultSet rs = stmt.executeQuery(sql);
        System.out.println("Top 5 Condiment items with highest " + nutrient + ":");
        while (rs.next()) {
    ...
    public void showItemInfo(String menuItemName) throws SQLException {
        String sql = "SELECT * FROM condiments_menu WHERE Item = '" + menuItemName + "'";
        ResultSet rs = stmt.executeQuery(sql);
        if (rs.next()) {
